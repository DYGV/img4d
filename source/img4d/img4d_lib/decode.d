...
ref auto ubyte[][] inverseFiltering(ref ubyte[][] data, bool gray = false)
{
    ...
    foreach (idx, scanline; rgb)
    {
        ...
        case Up:
            ...
            actualData[idx] = (temp[] += actualData[upIdx][]).map!(a => a.normalizePixelValue)
                .array
                .to!(ubyte[]);
        ...
        case Average:
            ...
            if (gray)
            {
                ...
                up.front[1 .. $].each!((o,
                        n) => temp ~= [((temp[o] + n) / 2 + sc[o]).normalizePixelValue].to!(
                        ubyte[]));
            }
            else
            {
                scanline.popFront;
                auto sc = scanline.join;
                up.front.each!((idx, n) => temp ~= [((n / 2) + current.front[0][idx])
        ...
        case Paeth:
            ...
            actualData[upIdx][0 .. lengthPerPixel].each!((idx,
                    a) => temp ~= [(a + joined[idx]).normalizePixelValue].to!(ubyte[]));

            actualData[upIdx][lengthPerPixel .. $].each!((i,
                    a) => temp ~= [(paethPredictor(temp[i], a,
                    actualData[upIdx][i]) + joined[i + lengthPerPixel]).normalizePixelValue].to!(
                    ubyte[]));
...
ref auto ubyte[][] parse(ref Header header, string filename)
{
    ...
    const string[] ancillaryChunks = [
        "tRNS", "gAMA", "cHRM", "sRGB", "iCCP", "tEXt", "zTXt", "iTXt", "bKGD",
        "pHYs", "vpAg", "sBIT", "sPLT", "hIST", "tIME", "fRAc", "gIFg", "gIFt",
        "gIFx", "oFFs", "pCAL", "sCAL"
    ];
    ...
    with (colorTypes) with (header)
    {
        ...
        return ((colorType != grayscale) && (colorType != grayscaleA)) ? uncChunks.inverseFiltering
