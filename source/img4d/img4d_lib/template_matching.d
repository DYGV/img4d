...
import img4d;
import std.array, std.math,std.algorithm.iteration, std.range;
...
class TemplateMatching{
	...
	int[] SSD(){
		int min_ssd = int.max;
		int xpos = 0, ypos = 0;
		foreach(i; (this.input_height-this.template_height).iota.parallel){
			...
			for(int j = 0; j < this.input_width - this.template_width; j++){
				...
				for(int h=0; h<this.template_height; h++){
					for(int w = 0; w < this.template_width; w++){
						int input_h = i+h;
						int input_w = j + w;
						vector += (this.inputImage[input_h][input_w] - ave_input) * (this.templateImage[h][w] - ave_template);
						double magnitude_1 = this.inputImage[input_h][input_w] - ave_input;
